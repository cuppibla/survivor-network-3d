...
Run: python setup_database.py --project=your-project-id
"""
...
def create_instance_with_enterprise(client, project_id):
    """Create a Spanner instance with ENTERPRISE edition using the admin API."""
    config_name = "projects/" + project_id + "/instanceConfigs/regional-us-central1"
...
def main():
    ...
    if instance_exists:
        print("Instance " + INSTANCE_ID + " already exists. Using existing instance.")
    elif not args.skip_instance:
        print("Creating instance " + INSTANCE_ID + " with ENTERPRISE edition...")
        operation = create_instance_with_enterprise(client, project_id)
    ...
    if database_exists:
        if args.force:
            print("Database " + DATABASE_ID + " exists. Deleting (--force specified)...")
            database.drop()
            print("Database deleted. Waiting 5 seconds...")
            time.sleep(5)
        else:
            print("Database " + DATABASE_ID + " already exists.")
            print("Use --force to delete and recreate it.")
            print("\nAccess your existing database at:")
            print("https://console.cloud.google.com/spanner/instances/" + INSTANCE_ID + "/databases/" + DATABASE_ID + "?project=" + project_id)
            return
    
    # Create database with schema
    print("Creating database " + DATABASE_ID + " with schema...")
